...
for noise in [0, 0.2, 0.4, 0.8]:
    ...
    if len(subset) == 1:
        ...
        plt.bar(
            bar_positions, subset.iloc[0][substance_columns], color=bar_color, width=5
        )  # width chosen for visual appeal
        ...
        plt.ylim(
            0, max_value * 1.1
        )  # Set y-axis limit to be 10% above the max value for a bit of headroom
