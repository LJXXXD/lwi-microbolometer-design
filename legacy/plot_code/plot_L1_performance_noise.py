...
df = pd.read_csv(
    file_name,
    converters={"Substance Comb": ast.literal_eval, "Basis Function Comb": ast.literal_eval},
)
...
for temp_K in temp_K_list:
    ...
    target_group = (
        temp_K,
        len(substance_ind_list),
        tuple(substance_ind_list),
        len(basis_func_ind_list),
        tuple(basis_func_ind_list),
        train_noise,
    )
    ...
    print(index)
    plt.plot(index, filtered_df["L1Loss"], label=f"NN with Temp={temp_K}")
