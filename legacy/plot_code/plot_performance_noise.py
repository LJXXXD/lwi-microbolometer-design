...
df = pd.read_csv(
    file_name,
    converters={"Substance Comb": ast.literal_eval, "Comb of Basis Functions": ast.literal_eval},
)
...
group_criteria_4train = [
    "Temperature_K",
    "Substance Number",
    "Substance Comb",
    "Basis Function Number",
    "Comb of Basis Functions",
    "Train Noise Max Percentage",
    "Test Aggregation",
]
...
substance_names = np.array(
    pd.read_excel("./data/Test 3 - 4 White Powers/white_powders_names.xlsx", header=None)
)
...
for substance in substance_list:
    ...
    for train_noise in train_noise_list:
        ...
        plt.plot(
            index,
            filtered_train_df[
                f"Substance {substance} skewnorm {confidence_perc * 100}% interval range"
            ],
            label=f"non PCA Train Noise={train_noise}",
        )
        plt.plot(
            index,
            filtered_train_df_pca[
                f"Substance {substance} skewnorm {confidence_perc * 100}% interval range"
            ],
            label=f"PCA Train Noise={train_noise}",
        )
        ...
        plt.title(
            f"{substance_names[substance, 1]} {confidence_perc * 100}% confidence interval range"
        )
        ...
        plt.plot(
            index,
            df_mtx_mul[f"Substance {substance} skewnorm {confidence_perc * 100}% interval range"],
            label="Linear Algebra (No normalization)",
        )
    ...
    plt.savefig(
        "noise output/"
        + file_name.split(".")[0]
        + f" Substance {substance} skewnorm\n{confidence_perc * 100}% interval range (Performance) vs Noise Level.png",
        dpi=300,
    )
    plt.close()
...
for train_noise in train_noise_list:
    ...
    index = range(len(filtered_train_df["Test Noise Max Percentage"]))
    plt.plot(index, filtered_train_df["MSELoss"], label="ML Model")
...
print(
    filtered_df_mtx_mul[f"Substance {substance} skewnorm {confidence_perc * 100}% interval range"]
)
