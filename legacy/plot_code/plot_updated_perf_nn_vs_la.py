...
for x, y in zip(updated_x_values, updated_y_values, strict=False):
    # if x <= :
...
for x, y in zip(updated_x_values2, updated_y_values2, strict=False):
    # if x <= :
...
for i, train_noise in enumerate([0]):
    ...
    plt.plot(
        subset["Test Noise"],
        subset["L1Loss"],
        label=f"Train Noise = {train_noise}",
        color=colors[i],
    )

# Plot the filtered additional data with updated legend name
plt.plot(
    filtered_additional_data["x"], filtered_additional_data["y"], label="Analytical", color="r"
)  # Magenta for analytical data


# Plot the filtered additional data with updated legend name
plt.plot(
    filtered_additional_data2["x"],
    filtered_additional_data2["y"],
    label="Updated Analytical (100 sampless)",
    color="b",
)  # Magenta for analytical data
