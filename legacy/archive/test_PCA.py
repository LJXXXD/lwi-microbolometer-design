...
air_trans, basis_funcs, spectra, substances_emit = load_data(
...
for temp_K_ind, temp_K in enumerate(temp_K_list):
    for num_basis_func_ind, num_basis_func in enumerate(range(1, len(basis_func_ind_list) + 1)):
        if num_basis_func != 4:
            continue
        basis_func_combs = list(
            itertools.combinations(range(len(basis_func_ind_list)), num_basis_func)
        )
            ...
            for agg in test_aggregation:
                for train_noise_perc in tqdm(train_noise_perc_list):
                    for test_noise_perc in test_noise_perc_list:
                        ...
                        history, models, best_model_index, avg_test_loss, pred_list, targ_list = (
                            train_val_test(dataset, train_params, sim_params)
                        )

                        loss_values = dict(
                            zip(["L1Loss", "RMSELoss", "MSELoss"], avg_test_loss, strict=False)
                        )
                        ...
                        for i, sub_ind in enumerate(substance_ind_list):
                            ...
                            for confidence_perc in (0.68, 0.95, 0.997):
                                ...
                                lower_quantile = dist.ppf(
                                    0.5 - confidence_perc / 2
                                )  # Lower quantile covering % of the data
                                upper_quantile = dist.ppf(
                                    0.5 + confidence_perc / 2
                                )  # Upper quantile covering % of the data

                                row[
                                    f"Substance {sub_ind} skewnorm {confidence_perc * 100}% low"
                                ] = lower_quantile
                                row[
                                    f"Substance {sub_ind} skewnorm {confidence_perc * 100}% high"
                                ] = upper_quantile
                                row[
                                    f"Substance {sub_ind} skewnorm {confidence_perc * 100}% interval range"
