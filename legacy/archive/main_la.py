...
from tools import NoisyDataset, Sim_Parameters, create_dataset, load_data
...
for temp_K_ind, temp_K in enumerate(temp_K_list):
    ...
    for substance_ind_list in sub_combs:
        for num_basis_func_ind, num_basis_func in enumerate(
            tqdm(range(1, len(basis_func_ind_list) + 1))
        ):
            ...
            for comb_ind, comb in enumerate(basis_func_combs):
                ...
                for idx in substance_ind_list:
                    ...
                    out = simulator(
                        sim_params, sub_signal
                    )  # Apply simulator() function to the column
                ...
                for train_noise in train_noise_list:
                    for test_noise in test_noise_list:
                        ...
                        row["MIN skewnorm 95% interval range"] = perc95_min
                        perc95_max = np.max(perc95_interval)
                        row["MAX skewnorm 95% interval range"] = perc95_max
