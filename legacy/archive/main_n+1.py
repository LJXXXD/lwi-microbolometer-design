...
air_trans, basis_funcs, spectra, substances_emit, substance_names = load_excel_data(
    file_paths["air_trans_file"],
        ...
        for train_noise in train_noise_list:
            for test_noise in test_noise_list:
                ...
                history, models, best_model_index, avg_test_loss, pred_list, targ_list = (
                    train_val_test(dataset, train_params)
                )
                loss_values = dict(zip(loss_func_names, avg_test_loss, strict=False))
                row = {
                ...
                for i, sub_ind in enumerate(substance_ind_list):
                    ...
                    for group_index, diff_value in enumerate(grouped_diffs):
                        row[f"Substance {sub_ind} Group {group_index}"] = np.mean(
                            np.abs(diff_value)
                        )
