...
if __name__ == "__main__":
    ...
    air_trans = np.array(
        pd.read_excel("./data/Test 2 - 21 Substances/Air transmittance.xlsx", header=None)
    )
    ...
    basis_funcs = np.array(
        pd.read_excel("./data/Test 2 - 21 Substances/Basis functions.xlsx", header=None)
    )
    ...
    substances_emit = np.array(
        pd.read_excel("./data/Test 2 - 21 Substances/substances.xlsx", header=None)
    )
    ...
    for i in range(substances_emit.shape[1]):
        ...
        out.append(
            simulator(spectra, mat_em, temp_K, air_trans, atm_dist_ratio, air_RI, basis_funcs)
        )
        # print(out[i])
