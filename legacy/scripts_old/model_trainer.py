...
for comb in tqdm(basis_func_combs, desc="Combs", leave=True):
    for temp_K in tqdm(get_list_values(config["temp_K_list"]), desc="Temps", leave=False):
        for atmdr in tqdm(get_list_values(config["atm_dist_ratio"]), desc="Dists", leave=False):
            ...
            for train_noise in tqdm(
                get_list_values(config["train_noise"]), desc="Noise", leave=False
            ):
                if train_noise != 0:
                    train_dataset = NoisyDataset(train_dataset, train_noise)
