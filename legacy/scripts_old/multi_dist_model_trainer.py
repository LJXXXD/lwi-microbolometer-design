...
for comb in basis_func_combs:
    multi_dist_train_dataset_list = []
    for temp_K in get_list_values(config["temp_K_list"]):
        for atmdr in tqdm(get_list_values(config["atm_dist_ratio"]), desc="Dists", leave=True):
            file_paths = config["file_paths"]
            sim_params = Sim_Parameters(
                air_trans=load_excel_data(file_paths["air_trans_file"]),
                air_RI=config["air_refractive_index"],
                atm_dist_ratio=atmdr,
            ...
            train_dataset, test_dataset = split_dataset(
                dataset, config["train_percentage"], config["random_flag"], config["random_seed"]
            )

            multi_dist_train_dataset_list.append(train_dataset.dataset)

        multi_dist_train_dataset = ConcatDataset(multi_dist_train_dataset_list)

        for train_noise in tqdm(
            get_list_values(config["train_noise"]), desc="Train Noise", leave=True
        ):
            if train_noise != 0:
                multi_dist_train_dataset = NoisyDataset(
                    multi_dist_train_dataset, train_params.train_noise
                )
            ...
            train_params = Train_Parameters(
                num_in=sim_params.basis_funcs.shape[1],
            ...
            history, models, best_model_index = train_val(multi_dist_train_dataset, train_params)
            ...
            row = {
                ...
                "atm_dist_ratio_start": config["atm_dist_ratio"]["start"],
                "atm_dist_ratio_stop": config["atm_dist_ratio"]["stop"],
                "atm_dist_ratio_step": config["atm_dist_ratio"]["step"],
                "percentage_step": config["percentage_step"],
                "train_percentage": config["train_percentage"],
                "batch_size": config["batch_size"],
                "learning_rate": config["learning_rate"],
...
df[["Substance Comb", "Basis Function Comb"]] = df[
    ["Substance Comb", "Basis Function Comb"]
].astype(str)
