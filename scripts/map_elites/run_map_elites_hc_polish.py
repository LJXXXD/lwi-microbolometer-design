#!/usr/bin/env python3
"""MAP-Elites + Hill-Climbing Polish.

Takes the top elites from a MAP-Elites archive and refines them using greedy
hill climbing with adaptive iteration budgets to converge to local optima.
"""
...
def main() -> None:
    """Run hill-climbing polish on MAP-Elites elites."""
    print("=" * 80)
    print("MAP-ELITES + HC POLISH: Refining Winners with Hill Climbing")
    print("=" * 80)
    print("Strategy: Take top MAP-Elites elites and polish with adaptive greedy")
    print("          hill climbing (more iterations for elites closer to target)")
    ...
    # ------------------------------------------------------------------
    # [3/4] Polish with hill climbing
    # ------------------------------------------------------------------
    print(f"\n[3/4] Polishing top {num_elites_to_polish} elites with adaptive hill climbing...")
    ...
    archive_path = output_dir / "map_elites_hc_archive.pkl"
    with archive_path.open("wb") as f:
        pickle.dump(archive, f)
    print(f"\nSaved archive to: {archive_path}")

    results_path = output_dir / "map_elites_hc_results.pkl"
    ...
    plot_map_elites_heatmap(
        ...
        grid_resolution=20,
        mu_range=(4.0, 20.0),
        output_path=output_dir / "map_elites_hc_heatmap.png",
    )
    ...
    for top_n in (10, 20, 50):
        plot_polished_elites(
            ...
            wavelengths=wavelengths_array,
            parameters_to_curves=gaussian_parameters_to_unit_amplitude_curves,
            output_path=output_dir / f"map_elites_hc_top{top_n}_elites.png",
            top_n=top_n,
            title_prefix="MAP-Elites + HC Polish",
        )
    ...
    print("\n" + "=" * 80)
    print("HC POLISH RESULTS: Before vs After (Top 50)")
    print("=" * 80)
    print(
        f"\n{'Rank':<6} | {'Before':>10} | {'After':>10} | {'Gain':>8} | "
        f"{'\u03bc\u2081':>6} | {'\u03bc\u2082':>6}"
    )
    print("-" * 64)
